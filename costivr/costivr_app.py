...
def costivr_app():
    ...
    def summary(allfile) :
        ...
        for i in allfile :
            ...
            data_hangup = data_hangup.groupby(['date','campaign']).agg(count=('No.', 'count')).reset_index()
            # cost['key'] = 1
            # rate['key'] = 1
            ...
            data_hangup = data_hangup.rename(columns={"count":"number_of_hangup_call (call)"})
            allcall = pd.concat([allcall, data_hangup], ignore_index=True)
        return allcall
